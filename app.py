...
for k, v in {
    ...
    "gtip_kirp": "",      # kırpılmış gtip kodu
    "yapistir_temizle": False,
}.items():
...
def taric_sorgula(gtip, ulke, tarih):
    try:
        with sync_playwright() as p:
            ...
            if tarih_temiz:
                # f-string yerine Playwright'ın güvenli evaluate kullan
                page.evaluate("(t) => { document.querySelector('#SimDatePic').value = t; }", tarih_temiz)
...
with sol:
    ...
    if yapistir.strip():
        # Tab ile mi yoksa satır satır mı geldi — ikisini de destekle
        ham = yapistir.strip()
        if "\t" in ham:
        ...
        if len(parcalar) >= 3:
            ...
            gtip  = st.session_state.gtip_kirp if st.session_state.gtip_kirp else gtip_cevir(gtip_ham)
            ulke  = ulke_cevir(ulke_ham)
            tarih = tarih_ham.replace(".", "-").replace("/", "-")

            # Tarih ve ülkeyi session'a kaydet (kırpma sorgusunda lazım)
            st.session_state.tarih_son = tarih
            st.session_state.ulke_son  = ulke

            # Dönüşüm önizleme
            kirp_notu = f" <span style='color:#dc2626;font-size:11px;'>✂️ kırpıldı</span>" if st.session_state.gtip_kirp else ""
            st.markdown(f"""
            <div style='background:#f0fdf4;border:1px solid #86efac;border-radius:6px;
                ...
                <span style='color:#4b5563;'>GTİP :</span> <b style='color:#166534;'>{gtip}</b>{kirp_notu}<br>
    ...
    if st.session_state.pdf_bytes:
        ...
        # PDF İndir — basınca kırpma bilgisini sıfırla (sıradaki sorgu için temiz başla)
    ...
    if st.session_state.sorgulandı:
        ...
        if st.button("✂️  Sondan 1 Rakam Sil", use_container_width=True):
            ...
            if len(mevcut) > 4:
                st.session_state.gtip_kirp = mevcut[:-1]
                # Otomatik tekrar sorgula
                html_content, pdf_bytes, current_url, hata = taric_sorgula(
                    st.session_state.gtip_kirp,
                ...
                if not hata:
                    ...
                    st.session_state.gtip_son  = st.session_state.gtip_kirp
                    st.session_state.durum     = f"✅ {st.session_state.gtip_kirp} / {st.session_state.ulke_son} ✂️"
                else:
