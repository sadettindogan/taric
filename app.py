...
def linkleri_cıkar(html):
    """TARIC sayfasındaki tıklanabilir GTİP linklerini çıkarır"""
    ...
    for a in soup.find_all("a", href=True):
        ...
        # TARIC measure/consultation linkleri
        if "taric" in href.lower() or "measures" in href.lower() or "consultation" in href.lower():
            if href.startswith("/"):
                href = "https://ec.europa.eu" + href
            if href.startswith("http"):
                linkler.append({"metin": metin, "url": href})
        # GTİP kodu gibi görünen linkler (sadece rakam/boşluk)
        elif metin and all(c in "0123456789 ." for c in metin) and len(metin.strip()) >= 4:
            if href.startswith("/"):
                href = "https://ec.europa.eu" + href
            if href.startswith("http"):
                linkler.append({"metin": metin, "url": href})
...
for k, v in {
    ...
    "navigate_url": "",
    "url_gecmisi": [],
    "linkler": [],        # sayfadaki tıklanabilir linkler
}.items():
...
with sol:
    ...
    if st.session_state.get("linkler"):
        ...
        st.markdown("<div style='font-size:10px;color:#888;letter-spacing:1px;font-weight:700;margin-bottom:4px;'>📎 SAYFADAKI LİNKLER</div>", unsafe_allow_html=True)
        for i, link in enumerate(st.session_state.linkler):
            if st.button(f"🔗 {link['metin']}", key=f"link_{i}", use_container_width=True):
                with sag:
                    with st.spinner(f"⏳ {link['metin']} yükleniyor..."):
                        h, pdf, hata = taric_url_ac(link["url"])
                    if not hata:
        ...
        with geri_col:
            if st.button("◀ Geri", use_container_width=True,
                ...
                with st.spinner("Yükleniyor..."):
                    h, pdf, hata = taric_url_ac(onceki_url)
                if not hata:
                    st.session_state.page_html = html_temizle(h)
                    st.session_state.pdf_bytes = pdf
                st.rerun()
